...
def quat2eul(qv):
    ...
    Translate between quaternion and Euler angle (ZYX) order representation of a rotation.
    Args:
        qv: Numpy array with three entries representing the vectorial component of the quaternion.
    Return:
        ax: X rotation angle in radians.
        ay: Y rotation angle in radians.
        az: Z rotation angle in radians.
    ...
    qv = np.reshape(qv, (4,))
    ax = np.arctan2(2*(qv[0]*qv[1] + qv[2]*qv[3]), 1-2*(qv[1]**2 + qv[2]**2))
    ay = np.arcsin(2*(qv[0]*qv[2] - qv[3]*qv[1]))
    az = np.arctan2(2*(qv[0]*qv[3] + qv[1]*qv[2]), 1-2*(qv[2]**2 + qv[3]**2))
...
def error_translation_2(t_pr, t_gt):
    t_pr = np.reshape(t_pr, (3,))
    t_gt = np.reshape(t_gt, (3,))

    return np.sqrt((np.square(t_gt - t_pr)))
...
def speed_score(t_pr, q_pr, t_gt, q_gt, applyThresh=False, rotThresh=0.5, posThresh=0.005):
    ...
    return speed, speed_t, speed_r, err_t

def speed_score_mean(t_pr, q_pr, t_gt, q_gt, applyThresh=True, rotThresh=0.5, posThresh=0.005):
    
    tr_error = error_translation(t_gt, t_pr)
    err_q = error_orientation(q_pr, q_gt) # [deg]

    return tr_error, err_q